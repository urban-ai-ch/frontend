...
export const getMaterialColor = (
  ...
  return (
    materialColors[datasetName.toLowerCase()]?.[material.toLowerCase()] ||
    materialColors[datasetName.toLowerCase()]?.["other"] ||
    materialColors["default"]?.["unknown"] ||
    "#000000"
  ); // Fallback black if no match is found
};

// Function to create dynamic styles for features
export const createFeatureStyle = (feature: any, datasetName: string, useDefaultColor: boolean) => {

	// If coloring is disabled, return a default gray style
  if (useDefaultColor) {
    return new Style({
      fill: new Fill({
        color: "#6699ff88", // Semi-transparent gray
      }),
      stroke: new Stroke({
        color: "#6699ff",
        width: 1,
  ...
  const material =
    ...
    (normalizedDatasetName === "materials"
      ? feature.get("main_facad")
      : null) || // Fallback for "materials"
